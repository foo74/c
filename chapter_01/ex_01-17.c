...
* by Foo74 -- March 21, 2016
*****************************************************/

/* Include the standard io library and define MAXLINE at 1000 which is
 * the max size of our arrays and also define PRINTTARGET at 80 which
 * is our goal. To print out all lines longer than 80 chars.
 */
#include <stdio.h>
#define MAXLINE 1000
#define PRINTTARGET 80
...
int main()
{
   ...
   printf("\nType in some lines, I will print out those more than 80 chars. Enter a '$' character to end.\n\n");
   ...
   while ((len = mygetline(line, MAXLINE)) > 0)
      ...
      if (len > max)
      {
         ...
         /* We want to print out any lines we find that are greater than our target. */
         if (len > PRINTTARGET)
            printf("\nfound a line greater than %d chars, it is: %s\n\n", PRINTTARGET, line); 
      }
